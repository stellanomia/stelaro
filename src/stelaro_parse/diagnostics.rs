...
impl<'dcx> DiagsParser {
    pub fn unexpected_token(
        ...
        diag.set_label(
            span,
            format!("不正なトークン`{}`が入力されました", unexpected),
        );
    ...
    pub fn prefix_increment(dcx: DiagCtxtHandle<'dcx>, span: Span) -> Diag<'dcx, ErrorEmitted> {
        let mut diag = dcx.struct_err(span);
        diag.set_code(ErrorCode::PrefixIncrement.into());
        diag.set_message("前置インクリメント".to_string());
        diag.set_label(
            span,
            "steraloは前置インクリメント演算子をもちません".to_string(),
        );
    ...
    pub fn unexpected_closing_delimiter(
        ...
        diag.set_label(
            span,
            "式の解析中に余分な閉じ括弧が見つかりました".to_string(),
        );
    ...
    pub fn unexpected_numeric_literal_for_identifier(
        ...
        diag.set_label(
            ...
            format!(
                "識別子を`{}`から始めることはできません",
                unexpected_literal.symbol.as_str()
            ),
        );
    ...
    pub fn cannot_use_underscore_as_identifier(
        ...
        diag.set_label(
            span,
            "`_` を識別子として使用することはできません".to_string(),
        );
...
mod tests {
    ...
    use crate::stelaro_parse::{
        diagnostics::ErrorCode, new_parser_from_source_str, parser::Parser,
    };
    ...
    fn src_to_parser<'a>(psess: &'a ParseSess, src: Rc<String>) -> Parser<'a> {
        new_parser_from_source_str(psess, "parse_test".into(), src.to_string()).unwrap()
    }
    ...
    fn test_chained_comparison() {
        let (sess, is_err) = get_sess_after_expr_parse("x = 1 < 2 < 3");
    ...
    fn test_expect_expression() {
        let (sess, is_err) = get_sess_after_expr_parse("1 + 2 - * 3");
    ...
    fn test_prefix_increment() {
        let (sess, is_err) = get_sess_after_expr_parse("y = ++x");
    ...
    fn test_unexpected_closing_delimiter() {
        let (sess, is_err) = get_sess_after_expr_parse("(a + b)) + 1");

        assert!(is_err);
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::UnexpectedClosingDelimiter.into())
        );
    }

    #[test]
    fn test_unexpected_token_for_identifier() {
        let (sess, is_err) = get_sess_after_stmt_parse("let if = 0;");

        assert!(is_err);
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::UnexpectedTokenForIdentifier.into())
        );
    }

    #[test]
    fn test_unexpected_token_for_type() {
        let (sess, is_err) = get_sess_after_item_parse("fn main(x: i32, y:i32): {");

        assert!(is_err);
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::UnexpectedTokenForType.into())
        );
    }
    ...
    fn unexpected_numeric_literal_for_identifier() {
        ...
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::UnexpectedNumericLiteralForIdentifier.into())
        );
    }
...
"#.trim()  // セミコロンなし
    ...
    fn test_missing_function_body() {
        let (sess, is_err) = get_sess_after_item_parse("fn f(x: i32,)");

        assert!(is_err);
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::MissingFunctionBody.into())
        );
    }
...
"#.trim()
        ...
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::CannotUseUnderscoreAsIdentifier.into())
        );
    }
...
"#.trim()
        ...
        assert!(
            sess.dcx()
                .has_err_code(ErrorCode::UnexpectedTokenForItem.into())
        );
    }
